...
def scale_galaxy():
    """Scale galaxy dimensions based on radio buble size (scale)."""
...
def detect_prob(disc_vol_scaled):
    ...
    if ratio < 0.002:  # set very low ratios to probability  of 0.
        detection_prob = 0
    elif ratio >= 5:   # set high ratios probability to 1
        detection_prob = 1
    else:
        detection_prob = -0.004754 * ratio**4 + 0.6674 *ratio**3 - 0.3599 * \
                        ratio**2 + 0.9198 * ratio + 0.009753
    return round(detection_prob, 3)

def random_polar_coordinates(disc_radius_scaled):
    """Generate uniform random (x, y) point wihtin a disc for 2D display."""
    r = random()
...
def spirals(b, r, rot_fac, fuz_fac, arm):
    ...
    spiral_stars = []
    fuzz = int(0.030 * abs(r))    # random shift star locations
...
def main():
    ...
    spirals(b = -0.3, r=disc_radius_scaled, rot_fac=1.91, fuz_fac=1.5, arm=0)
    spirals(b = -0.3, r=disc_radius_scaled, rot_fac=2, fuz_fac=1.5, arm=0)
    spirals(b = -0.3, r=disc_radius_scaled, rot_fac=-2.09, fuz_fac=1.5, arm=0)
    spirals(b = -0.3, r=disc_radius_scaled, rot_fac=0.5, fuz_fac=1.5, arm=0)
    spirals(b = -0.3, r=disc_radius_scaled, rot_fac=0.4, fuz_fac=1.5, arm=0)
    spirals(b = -0.3, r=disc_radius_scaled, rot_fac=-0.5, fuz_fac=1.5, arm=0)
    spirals(b = -0.3, r=disc_radius_scaled, rot_fac=-0.6, fuz_fac=1.5, arm=0)
