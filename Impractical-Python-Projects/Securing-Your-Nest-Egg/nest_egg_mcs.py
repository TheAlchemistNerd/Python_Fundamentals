...
print("\nNote: Input data sould be in percent, not decimal!\n")
try:
    ...
    blend_50_50 = read_to_list('S_B_blend_1926-2013_pct.txt')
    infl_rate = read_to_list('annual_inf_rate_1926-2013_pct.txt')
except IOError as e:
...
print("sbc_blend = 40% SP500/50% TBond")
print("Press ENTER to take default value shown in [brackets]. \n")

# get user input
investment_type = default_input("Enter investment type: (stocks, bonds, sb_blend, "\
                                " sbc_blend): \n", 'bonds').lower()
...
def montecarlo(returns):
	...
	while case_count < int(num_cases):
		...
		lifespan_infl = []
		for i in lifespan:
			lifespan_returns.append(returns[1 % len(returns)])
			lifespan_infl.append(infl_rate[1 % len(infl_rate)])
		...
		for index, i in enumerate(lifespan_returns):
			...
			if index == 0:
				...
				withdrawal_infl_adj = int(withdrawal*(1 + infl))
...
def main():
	"""Call ,MCS & bankrupt function and draw bar charts of results."""
